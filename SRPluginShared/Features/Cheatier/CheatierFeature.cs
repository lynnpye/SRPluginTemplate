    ...
    public class CheatierFeature : FeatureImpl
    {
        ...
        public CheatierFeature()
            : base(
                ...
                [
                    (
                        CICheatier = new ConfigItem<bool>(
                            PLUGIN_FEATURES_SECTION,
                            nameof(Cheatier),
                            true,
                            "adds a new, cheatier, cheat bar"
                        )
                    )
                ],
                new List<PatchRecord>(
                    PatchRecord.RecordPatches(
                        AccessTools.Method(
                            typeof(DebugConsolePatch),
                            nameof(DebugConsolePatch.AfterDebugInputReversePatch)
                        ),
                        AccessTools.Method(
                            typeof(DebugConsolePatch),
                            nameof(DebugConsolePatch.DrawCheatBarPrefix)
                        ),
                        AccessTools.Method(
                            typeof(PDAPatch),
        ...
        {
            get => CICheatier.GetValue();
            set => CICheatier.SetValue(value);
        }
        ...
        internal class PDAPatch
        {
            ...
            {
                bool flag = false;
                Dictionary<int, bool> workingMap =
                    AccessTools.Field(typeof(TurnDirector), "workingMap").GetValue(td)
                ...
                if (player != null && workingMap[player.actorUID])
                {
                    ...
                    if (td.FocusedPlayer == player)
                    {
                        ...
                        if (!td.tryingToAdvanceTeam && !td.GameOver)
                        {
                            ...
                            {
                                AccessTools
                                    .Field(typeof(TurnDirector), "previousFocusedPlayer")
                                    .SetValue(td, td.FocusedPlayer);
                            }

                            AccessTools
                                .Field(typeof(TurnDirector), "focusedPlayer")
                                .SetValue(td, null);
                            //focusedPlayer = td.grid.allPlayers.Find((Player x) => workingMap[x.actorUID]);
                            if (
                                td.FocusedPlayer == null
                                && td.machine.IsState(DirectorState.PLAYERREADY)
                            )
                            {
                                _ = td.ChooseNextPlayer();
                            }
                        }
                    }
                    flag = true;
            ...
            {
                DeworkPlayer(
                    SceneSingletonBehavior<TurnDirector>.Instance.FocusedPlayer,
                    SceneSingletonBehavior<TurnDirector>.Instance
                );
                yield return true;

                SceneSingletonBehavior<TurnDirector>.Instance.WorkPlayer(
                    SceneSingletonBehavior<TurnDirector>.Instance.PlayerZero
                );
                yield return true;

#if SRR
                SceneSingletonBehavior<HUDManager>.Instance.actionPanel.SetPanel(
                    SceneSingletonBehavior<TurnDirector>.Instance.PlayerZero
                );
...
#else
                SceneSingletonBehavior<HUDManager>.Instance.weaponPanel.SetPanel(
                    SceneSingletonBehavior<TurnDirector>.Instance.PlayerZero
                );
            ...
            public static void CloseEquipScreenPostfix()
            {
                ...
                if (OnPlayerChangedOnPDAEquipScreenClosed)
                {
                    ...
                    _ = SceneSingletonBehavior<TurnDirector>.Instance.StartCoroutine(
                        StaggeredFix()
                    );
                }
            }
        ...
        internal class DebugConsolePatch
        {
            ...
            public static void DrawCheatBarPrefix(
                DebugConsole __instance,
                float ___butHeight,
                bool ___showCheats
            )
            {
                ...
                try
                {
                    float num = Screen.width * 0.75f;
                    float left = Screen.width - num - 10f;
                    ...
                    if (___showCheats && RunManager.HasInstance())
                    {
                        ...
                        )
                        {
                            ...
                            {
                                Logger.Log(
                                    LogChannel.CONSOLE_DESIGNER,
                                    LogLevel.Info,
                                    "Opened Equipment Screen (gear drop!) "
                                );
                        ...
                        )
                        {
                            ...
                            if (player != null)
                            {
                                StatsUtil.SetAttribute(
                                    player,
                                    global::isogame.Attribute.Attribute_AP,
                                    player.baseAttributes.ap + 1
                                );
                                RunManager.Instance.DirectAddAP(1, null);
                                Logger.Log(
                                    LogChannel.CONSOLE_DESIGNER,
                                    LogLevel.Info,
                                    "APMax add 1 (you but better) "
                                );
                                LazySingletonBehavior<Analyzer>.Instance.CountCheat(1);
                            }
                            LazySingletonBehavior<InputManager>.Instance.ConsumeFrame();
                        ...
                        )
                        {
                            ...
                            Logger.Log(
                                LogChannel.CONSOLE_DESIGNER,
                                LogLevel.Info,
                                "AP add 100 (just get it over with already) "
                            );
                            LazySingletonBehavior<Analyzer>.Instance.CountCheat(1);
                            LazySingletonBehavior<InputManager>.Instance.ConsumeFrame();
                        }
                        ...
                        )
                        {
                            ...
                            if (player != null)
                            {
                                ...
                                Logger.Log(
                                    LogChannel.CONSOLE_DESIGNER,
                                    LogLevel.Info,
                                    "HPMax add 50 (bigger and tougher) "
                                );
                                LazySingletonBehavior<Analyzer>.Instance.CountCheat(1);
                            }
                            LazySingletonBehavior<InputManager>.Instance.ConsumeFrame();
                        ...
                        )
                        {
                            ...
                            Logger.Log(
                                LogChannel.CONSOLE_DESIGNER,
                                LogLevel.Info,
                                "HP add 500 (once more, with feeling) "
                            );
                            LazySingletonBehavior<Analyzer>.Instance.CountCheat(1);
                            LazySingletonBehavior<InputManager>.Instance.ConsumeFrame();
                        }
                        ...
                        )
                        {
                            ...
                            Logger.Log(
                                LogChannel.CONSOLE_DESIGNER,
                                LogLevel.Info,
                                "Karma add 1000 (it was very effective)"
                            );
                            LazySingletonBehavior<Analyzer>.Instance.CountCheat(1);
                            LazySingletonBehavior<InputManager>.Instance.ConsumeFrame();
                        }
...
#if !SRR
                    bool flag =
                        LazySingletonBehavior<Analyzer>.HasInstance()
                        && LazySingletonBehavior<Analyzer>.Instance.CurrentGameCheated();
                    string text4 = (!___showCheats) ? "< Cheatier" : "> Cheatier";
                    if (
                        GUILayout.Button(
                            text4,
                            (!flag) ? __instance.styleTwo : __instance.styleOne,
                            new GUILayoutOption[]
                            {
                                GUILayout.Width(88f),
                                GUILayout.Height(___butHeight)
